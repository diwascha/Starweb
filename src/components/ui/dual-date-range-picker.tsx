...
export function DualDateRangePicker({ selected, onSelect }: DualDateRangePickerProps) {
  ...
  const handleADSelect = (range?: DateRange) => {
    onSelect(range);
    if (range?.from) {
      setDisplayDateAD(range.from);
      const newNepaliDate = new NepaliDate(range.from);
